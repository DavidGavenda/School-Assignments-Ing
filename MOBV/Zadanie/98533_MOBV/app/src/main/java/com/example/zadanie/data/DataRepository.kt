...
class DataRepository private constructor(
    private val service: RestApi, private val cache: LocalCache
) {
    ...
    suspend fun apiBarCheckin(
        bar: NearbyBar, onError: (error: String) -> Unit, onSuccess: (success: Boolean) -> Unit
    ) {
        try {
            val resp = service.barMessage(
                BarMessageRequest(
                    bar.id.toString(), bar.name, bar.type, bar.lat, bar.lon
                )
            )
            if (resp.isSuccessful) {
                resp.body()?.let { _ ->
        ...
        try {
            ...
            if (resp.isSuccessful) {
                resp.body()?.let { bars ->
                    ...
                    val b = bars.map {
                        BarItem(
                            it.bar_id, it.bar_name, it.bar_type, it.lat, it.lon, it.users
                        )
                    }
                    cache.deleteBars()
                    cache.insertBars(b)
    ...
    suspend fun apiNearbyBars(
        lat: Double, lon: Double, onError: (error: String) -> Unit
    ): List<NearbyBar> {
    ...
    suspend fun apiBarDetail(
        id: String, onError: (error: String) -> Unit
    ): NearbyBar? {
    ...
    fun dbSortedByDIST(sort: Sort, location: MyLocation): LiveData<List<BarItem>?> {
        return cache.getBarsSortedByDIST(sort, location)
    }
    ...
    companion object {
        ...
        fun getInstance(service: RestApi, cache: LocalCache): DataRepository =
            INSTANCE ?: synchronized(this) {
                INSTANCE ?: DataRepository(service, cache).also { INSTANCE = it }
            }
        ...
        try {
            ...
            if (resp.isSuccessful) {
                resp.body()?.let { friends ->
                    ...
                    val f = friends.map {
                        FriendItem(
                            it.user_id, it.user_name, it.bar_id, it.bar_name
                        )
                    }
                    cache.deleteFriends()
    ...
    suspend fun addFriend(
        username: String, onError: (error: String) -> Unit, onStatus: (status: Boolean) -> Unit
    ) {
        try {
    ...
    suspend fun deleteFriend(
        ...
        try {
            val resp = service.deleteFriend(DeleteFriendRequest(contact = username))
            if (resp.isSuccessful) {
                onStatus(true)
            } else if (resp.code() == 500) {
                onStatus(false)
                onError("Failed to delete friend, username does not exist")
            }
        } catch (ex: IOException) {
            ex.printStackTrace()
            onError("Failed to delete friend, check internet connection")
        } catch (ex: Exception) {
            ex.printStackTrace()
